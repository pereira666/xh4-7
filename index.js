		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '❌ Este comando so pode ser usado pelo bot proprietario! ❌',
					admin: '❌ Este comando sobpode ser usado por administradores de grupo! ❌',
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Essa e a lista de bloqueados:\n'
				...
				case 'ep':
					...
					break
				case 'tahta':
					if (args.length < 1) return reply('O texto é om')
				...
				case 'gtts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Ultrapassou o limite de texto')
				...
				case 'hilih':
					...
					break*/
				case 'yt2mp3':
					if (args.length < 1) return reply('Cada a url, hum?')
				...
				case 'tiktokstalk':
					try {
						...
						reply('Impossivel nome de usuario invalido')
					}
					break
				case 'nulis':
				case 'tulis':
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Cade a url, hum?')
				...
				case 'clearall':
					...
					reply('O Loser deletou todo o chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem e Voce?')
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar destino, talvez porque seja privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ o adesivo de resposta é um ❌')
					reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativar com sucesso o modo simi no grupo✔️')
